...
**Status**: R1 — ingest trio + revenue beat run offline from committed fixtures (Founder 1/1
bootstrap, ADR-2607180900); R2/R3 Council-gated. Current state and history: `MATURITY.md`.
**Run it**: `docs/operator-quickstart.md` (tests, heartbeats, chain verification, the R1 observe).
...
## Running it (R1, offline)

What runs from this repo, with no network and no Council: the test suite
(`./run_tests_clj.sh`, 16 suites under bb), the procurement heartbeat
(`methods/autorun.cljk` → append-only content-addressed Datom log, chain
verification, tamper detection), the per-yen revenue trace
(`methods/revenue_ledger.cljk`), and the R1 observe (`methods/mesh.cljk`,
gated on `DANJO_R1_COUNCIL_RATIFY_TX_HASH`). Every command, with the output it
printed, is in **`docs/operator-quickstart.md`**. The Pregel cells in the table
above are still path-reserved: R2/R3 (cross-reference engine, named-party
observations, oversight reports) do not run from here.

Decisions taken in this repo since it left the monorepo: `docs/adr/`.

## Related Files

In this repo:

- `manifest.edn`, `CLAUDE.md`, `MATURITY.md`, `REVENUE-LEDGER.md`
- `methods/` (implementation + tests side by side; open, versioned detector heuristics —
  `v1-jp-seed.edn`, 7 methods; G6 open method)
- `data/` (representative fixtures; `data/REVENUE-COVERAGE.md` is generated by
  `methods/coverage.cljk`), `registry/` (worldwide fiscal-source seed, G14 tiers)
- `docs/operator-quickstart.md`, `docs/adr/`

In the etzhayyim monorepo (`etzhayyim/root`; the west sibling `orgs/etzhayyim/root`).
The paths below are monorepo paths, kept as they were written:

- `/00-contracts/lexicons/com/etzhayyim/danjo/` (4 Lexicons + README — `test_charter_gates`
  reads these; see `run_tests_clj.sh` for how it finds them)
